// @codekit-prepend '../../node_modules/d3/d3.min.js';
/* global ns360, News360, ga, d3 */
...
$(document).ready(function ($) {
    ...
    var currentContactForm = null;

    var invalidEmailText = 'Please enter a valid email address';
    var errorColor = '#dc1919';
    var emailSubmitFailureText = 'Email sending failure, please try again';

    var successMessage = 'Thank you. We will get in touch soon.';
    ...
    function userChangedForm(activeContactForm) {
        if (currentContactForm) {
            if (currentContactForm[0] !== activeContactForm[0]) {
                return true;
            }
        }
        return false;
    ...
    function hiddenInputFirstClick(parentElement) {
        parentElement.find('.promo-field-container').removeClass('hidden');
        var input = parentElement.find('.promo-field');
        input.removeClass('hidden');
        input.focus();
    }


    function permanentInputFirstClick(parentElement) {
        var input = parentElement.find('.email-input');
        input.focus();

        if (input.val()) {
            handleEmailSubmission(parentElement);
        }
    }


    function isInputPermanent(contactForm) {
        return contactForm.hasClass('permanent-input')
    }

    function resetContactForm(contactForm) {
        contactForm.find('.email-input').val('');
        contactForm.find('.email-submit-message').html('');

        if (!isInputPermanent(contactForm)) {
            contactForm.find('.promo-field-container').addClass('hidden');
            contactForm.find('.promo-field').addClass('hidden');
            contactForm.find('.email-submit-message-container').addClass('hidden');
        }
    }

    $('.demo-request-form').submit(function (e) {
        e.preventDefault();
        ga('send', 'event', 'get demo button clicked');
        var activeContactForm = $(this).parent();
        if (userChangedForm(activeContactForm)) {
            resetContactForm(currentContactForm);
            numberOfClicks = 0;
        }
        currentContactForm = activeContactForm;

        if (numberOfClicks === 0) {
            if (isInputPermanent(activeContactForm)) {
                permanentInputFirstClick(activeContactForm);
            } else {
                hiddenInputFirstClick(activeContactForm);
            }
        } else {
            handleEmailSubmission(activeContactForm);
        }
        numberOfClicks++;
    });


    function handleEmailSubmission(contactFormElem) {
        var input = contactFormElem.find('.email-input');
        var messageContainer = contactFormElem.find('.email-submit-message-container');
        ...
        if (isEmail(inputValue)) {
            postRequestForDemo(contactFormElem);
        } else {
            displayMessage(messageContainer, invalidEmailText, errorColor)
        }
    }

    function displayMessage(messageContainer, text, color) {
        messageContainer.find('.email-submit-message').html(text);
        messageContainer.removeClass('hidden');
        messageContainer.css('color', color);
    }

    function postRequestForDemo(parentElement) {
        var messageContainer = parentElement.find('.email-submit-message-container');
        var postData = {
            'email': parentElement.find('.email-input').val(),
            'url': window.location.href
        };

        $.ajax({
            url: 'https://app.native.ai/api/v2/demo_request',
            method: 'POST',
            ...
            displayMessage(messageContainer, successMessage, successColor);
            numberOfClicks = 0;
        }).fail(function (xhr) {
            displayMessage(messageContainer, emailSubmitFailureText, errorColor);
            console.log(xhr);
        });
...
treemap.setNavigationCallback(function () {
    updateNavigationBar();
});
